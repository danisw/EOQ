...
loadjs.ready("head", function () {
    ...
    fpemesananedit = currentForm = new ew.Form("fpemesananedit", "edit");
    ...
    fpemesananedit.addFields([
        ["id_pesanan", [fields.id_pesanan.visible && fields.id_pesanan.required ? ew.Validators.required(fields.id_pesanan.caption) : null], fields.id_pesanan.isInvalid],
    ...
    // Validate form
    fpemesananedit.validate = function () {
    ...
    // Form_CustomValidate
    fpemesananedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpemesananedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fpemesananedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpemesananedit" id="fpemesananedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_pesanan->Visible) { // id_pesanan ?>
    <div id="r_id_pesanan" class="form-group row">
        <label id="elh_pemesanan_id_pesanan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_pesanan->caption() ?><?= $Page->id_pesanan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_pesanan->cellAttributes() ?>>
<span id="el_pemesanan_id_pesanan">
<span<?= $Page->id_pesanan->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_pesanan->getDisplayValue($Page->id_pesanan->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pemesanan" data-field="x_id_pesanan" data-hidden="1" name="x_id_pesanan" id="x_id_pesanan" value="<?= HtmlEncode($Page->id_pesanan->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
