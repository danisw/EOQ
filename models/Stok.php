...
/**
 * Table class for stok
 */
class Stok extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // id_barang
        $this->id_barang = new DbField('stok', 'stok', 'x_id_barang', 'id_barang', '`id_barang`', '`id_barang`', 3, 11, -1, false, '`id_barang`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // curret_qty
        $this->curret_qty = new DbField('stok', 'stok', 'x_curret_qty', 'curret_qty', '`curret_qty`', '`curret_qty`', 3, 11, -1, false, '`curret_qty`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->curret_qty->Nullable = false; // NOT NULL field
        $this->curret_qty->Required = true; // Required field
        $this->curret_qty->Sortable = true; // Allow sort
        $this->curret_qty->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->curret_qty->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->curret_qty->Param, "CustomMsg");
        $this->Fields['curret_qty'] = &$this->curret_qty;

        // last_update
        $this->last_update = new DbField('stok', 'stok', 'x_last_update', 'last_update', '`last_update`', CastDateFieldForLike("`last_update`", 0, "DB"), 135, 19, 0, false, '`last_update`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        $this->last_update->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->last_update->Param, "CustomMsg");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`stok`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->curret_qty->DbValue = $row['curret_qty'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("stoklist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "stokedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "stokadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("stokview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("stokview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "stokadd?" . $this->getUrlParm($parm);
        } else {
            $url = "stokadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("stokedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("stokadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("stokdelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->curret_qty->EditValue = $this->curret_qty->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->curret_qty);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->curret_qty);
