...
class PegawaiEdit extends Pegawai
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_pegawai") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_pegawai->setQueryStringValue($keyValue);
                $this->id_pegawai->setOldValue($this->id_pegawai->QueryStringValue);
            } elseif (Post("id_pegawai") !== null) {
                $this->id_pegawai->setFormValue(Post("id_pegawai"));
                $this->id_pegawai->setOldValue($this->id_pegawai->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_pegawai") ?? Route("id_pegawai")) !== null) {
                    $this->id_pegawai->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_bagian->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->id_bagian->Visible = false; // Disable update for API request
            } else {
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_pegawai->Required) {
            if (!$this->id_pegawai->IsDetailKey && EmptyValue($this->id_pegawai->FormValue)) {
                $this->id_pegawai->addErrorMessage(str_replace("%s", $this->id_pegawai->caption(), $this->id_pegawai->RequiredErrorMessage));
            }
        }
        ...
        if ($this->nama_pegawai->Required) {
            if (!$this->nama_pegawai->IsDetailKey && EmptyValue($this->nama_pegawai->FormValue)) {
                $this->nama_pegawai->addErrorMessage(str_replace("%s", $this->nama_pegawai->caption(), $this->nama_pegawai->RequiredErrorMessage));
            }
        }
        if ($this->alamat_pegawai->Required) {
            if (!$this->alamat_pegawai->IsDetailKey && EmptyValue($this->alamat_pegawai->FormValue)) {
                $this->alamat_pegawai->addErrorMessage(str_replace("%s", $this->alamat_pegawai->caption(), $this->alamat_pegawai->RequiredErrorMessage));
            }
        }
        if ($this->hp_pegawai->Required) {
            if (!$this->hp_pegawai->IsDetailKey && EmptyValue($this->hp_pegawai->FormValue)) {
                $this->hp_pegawai->addErrorMessage(str_replace("%s", $this->hp_pegawai->caption(), $this->hp_pegawai->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // alamat_pegawai
            $this->alamat_pegawai->setDbValueDef($rsnew, $this->alamat_pegawai->CurrentValue, "", $this->alamat_pegawai->ReadOnly);

            // hp_pegawai
            $this->hp_pegawai->setDbValueDef($rsnew, $this->hp_pegawai->CurrentValue, "", $this->hp_pegawai->ReadOnly);

            // id_bagian
            if ($Security->canAdmin()) { // System admin
                $this->id_bagian->setDbValueDef($rsnew, $this->id_bagian->CurrentValue, 0, $this->id_bagian->ReadOnly);
            }
