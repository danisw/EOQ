 ...
 * Page class
 */
class PemesananEdit extends Pemesanan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PemesananEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_pesanan") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_pesanan->setQueryStringValue($keyValue);
                $this->id_pesanan->setOldValue($this->id_pesanan->QueryStringValue);
            } elseif (Post("id_pesanan") !== null) {
                $this->id_pesanan->setFormValue(Post("id_pesanan"));
                $this->id_pesanan->setOldValue($this->id_pesanan->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_pesanan->IsDetailKey) {
            $this->id_pesanan->setFormValue($val);
        }
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_pesanan->Required) {
            if (!$this->id_pesanan->IsDetailKey && EmptyValue($this->id_pesanan->FormValue)) {
                $this->id_pesanan->addErrorMessage(str_replace("%s", $this->id_pesanan->caption(), $this->id_pesanan->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // nama_pemesan
            $this->nama_pemesan->setDbValueDef($rsnew, $this->nama_pemesan->CurrentValue, "", $this->nama_pemesan->ReadOnly);

            // id_barang
            $this->id_barang->setDbValueDef($rsnew, $this->id_barang->CurrentValue, 0, $this->id_barang->ReadOnly);

            // jumlah_pesanan
            $this->jumlah_pesanan->setDbValueDef($rsnew, $this->jumlah_pesanan->CurrentValue, "", $this->jumlah_pesanan->ReadOnly);

            // lead_time
            $this->lead_time->setDbValueDef($rsnew, $this->lead_time->CurrentValue, 0, $this->lead_time->ReadOnly);

            // pakai
            $this->pakai->setDbValueDef($rsnew, $this->pakai->CurrentValue, "", $this->pakai->ReadOnly);
