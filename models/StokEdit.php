...
class StokEdit extends Stok
{
    ...
    // Table name
    public $TableName = 'stok';

    // Page object name
    public $PageObjName = "StokEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (stok)
        if (!isset($GLOBALS["stok"]) || get_class($GLOBALS["stok"]) == PROJECT_NAMESPACE . "stok") {
            $GLOBALS["stok"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'stok');
        }
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_barang") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_barang->setQueryStringValue($keyValue);
                $this->id_barang->setOldValue($this->id_barang->QueryStringValue);
            } elseif (Post("id_barang") !== null) {
                $this->id_barang->setFormValue(Post("id_barang"));
                $this->id_barang->setOldValue($this->id_barang->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("stoklist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "stoklist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'curret_qty' first before field var 'x_curret_qty'
        $val = $CurrentForm->hasValue("curret_qty") ? $CurrentForm->getValue("curret_qty") : $CurrentForm->getValue("x_curret_qty");
        if (!$this->curret_qty->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->curret_qty->Visible = false; // Disable update for API request
            } else {
                $this->curret_qty->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->curret_qty->CurrentValue = $this->curret_qty->FormValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->curret_qty->setDbValue($row['curret_qty']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // curret_qty
            $this->curret_qty->ViewValue = $this->curret_qty->CurrentValue;
            $this->curret_qty->ViewValue = FormatNumber($this->curret_qty->ViewValue, 0, -2, -2, -2);
            $this->curret_qty->ViewCustomAttributes = "";
            ...
            // curret_qty
            $this->curret_qty->LinkCustomAttributes = "";
            $this->curret_qty->HrefValue = "";
            $this->curret_qty->TooltipValue = "";
            ...
            // curret_qty
            $this->curret_qty->EditAttrs["class"] = "form-control";
            $this->curret_qty->EditCustomAttributes = "";
            $this->curret_qty->EditValue = HtmlEncode($this->curret_qty->CurrentValue);
            $this->curret_qty->PlaceHolder = RemoveHtml($this->curret_qty->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->curret_qty->Required) {
            if (!$this->curret_qty->IsDetailKey && EmptyValue($this->curret_qty->FormValue)) {
                $this->curret_qty->addErrorMessage(str_replace("%s", $this->curret_qty->caption(), $this->curret_qty->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->curret_qty->FormValue)) {
            $this->curret_qty->addErrorMessage($this->curret_qty->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // curret_qty
            $this->curret_qty->setDbValueDef($rsnew, $this->curret_qty->CurrentValue, 0, $this->curret_qty->ReadOnly);

            // last_update
            $this->last_update->setDbValueDef($rsnew, UnFormatDateTime($this->last_update->CurrentValue, 0), CurrentDate(), $this->last_update->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("stoklist"), "", $this->TableVar, true);
        $pageId = "edit";
